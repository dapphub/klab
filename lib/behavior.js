  ...
  while (frontier.length > 0) {
    ...
    frontier.forEach((node, i) => {
      ...
      if(!seen_nodes[node.id] && steps.length == 1) {
        ...
        next_frontier = next_frontier.concat(node);
      }
      if(!seen_nodes[node.id] && steps.length > 1) {
        ...
        let minDeltaC = steps
          .map((step, i) => ({
            length: kast.format(getDeltaC(step)).length,
            index: i,
            deltaCRaw: getDeltaC(step)
          }))
        let isBinaryBranch = minDeltaC.length == 2
        let minDeltaCIndex = isBinaryBranch
          && (minDeltaC[0].length < minDeltaC[1].length ? 0 : 1)
        let minDeltaCRaw = isBinaryBranch
          && (minDeltaC[0].length < minDeltaC[1].length
            ? minDeltaC[0].deltaCRaw
            : minDeltaC[1].deltaCRaw
            )
        let children = steps.map((step, i) => {
          let deltaCRaw = getDeltaC(step)
          let deltaC = minDeltaCRaw
            ? (isBinaryBranch && minDeltaCIndex != i ? "~ " : "") + kast.format(isBinaryBranch && minDeltaCIndex != i ? minDeltaCRaw : deltaCRaw).split("\n").filter(s => !!s).join('\n')
            : ""
            ;
          return {
            index: i,
            ...
            endheight: height,
            step,
            deltaC: deltaC.replace(/~ ~ /g,""),
            deltaCRaw
          }
        });
        node.children = children;
        next_frontier = next_frontier.concat(children)
      }
