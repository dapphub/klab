...
const makePrelude = config => {
  ...
  if(smt_prelude_old != smt_prelude) {
    console.log("write " + prelude_out_path);
...
const buildAct = config => ({act, oog, pass, name}) => {
  ...
  const getTerm = ({
    ...
    const buildAccount = varname => {
        let storage_pass = "_"
        Object.keys(act.storage[varname])
            .forEach(key => {
                storage_pass = `store(${storage_pass}, ${clean(key)}, (${clean(act.storage[varname][key][0])} => {clean(act.storage[varname][key][1])}))`
            })
        let storage_fail = "_"
        Object.keys(act.storage[varname])
            .forEach(key => {
                storage_fail = `store(${storage_fail}, ${clean(key)}, ${clean(act.storage[varname][key][0])}) => _`
            })
        let storage = (pass && !oog) ? storage_pass : storage_fail
/*      let fstore = Object.keys(act.storage[varname])
      .map(key => storageEntry(varname, key, act.storage[varname][key], pass && !oog))
      .concat(["_:Map"])
      .reverse()
...
*/
  ...
  // GAS
  const gas = "VGas => " + ((act.hasGas && !oog) ? "VGas -Int " + act.gas : "_");
  const gasCond = "VGas " + (oog ? "<Int " : ">=Int ") + act.gas
  ...
  const storageEntry = (subject, key, vals, pass) => {
    if(!pass) vals = [vals[0]]
      .concat(
        vals.length > 1
        ? ["_"]
    ...
    subject = subject == "ACCT_ID" ? act.subject : subject;
