...
const buildAct = config => ({act, oog, pass, name, hasGas, gas, gas_raw}) => {
  ...
  const buildAccount = varname => {
    let storage_pass = Object.keys(act.storage[varname])
      .map(key => `[${storagekeys(realname(varname), key)} <- (${remember_junk(clean(act.storage[varname][key][0]))} => ${clean(act.storage[varname][key][1])})]`)
      .join('        \n');
    let storage_fail = Object.keys(act.storage[varname])
      .map(key => `[${storagekeys(realname(varname), key)} <- (${remember_junk(clean(act.storage[varname][key][0]))} => _)]`)
      .join('        \n');
