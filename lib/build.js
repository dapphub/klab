...
const parseAct = config => (act_str, silent = false) => {
  ...
  let act = act_arr
    .reduce((a, e) => {
      ...
      } else if(/^macros/.test(head)) {
          a.macros = tail
              .map(l => l.trim())
              .filter(l => l != "")
              .map(l => mapInterface(a.interface, toK(l).trim()))
              .map(toK)
              .map(l => [`syntax Int ::= "${l.split("=>")[0].trim()}" [function]\nrule ${l} [macro]`])
    ...
    Object.keys(act_proofs)
      .forEach(name => {
        ...
        if (toClose) {
          ...
          const module = kjson.renderModule(_rules, rule.name, rule.act.macros) // TODO maybe skip this
          const hash   = config.get_proof_hash({name: rule.name, spec: module});
          // const gas_names = (gas_module.length > 0 && [hash] || [])
          rule.module  = kjson.renderModule(_rules, hash, rule.act.macros);
          rule.hash    = hash;
