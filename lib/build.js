...
const buildActs = (config, act_collection) => {
  ...
  const getStatus = rule =>
    testPath(path.join(KLAB_OUT, "pass", rule.hash)) && 'pass'
      || testPath(path.join(KLAB_OUT, "fail", rule.hash)) && 'fail'
      || '????'
      ...
      .forEach(name => {
        ...
        let toClose = !rule.closed && rule.imports
          .map(rule_name =>
            ...
            && act_proofs[rule_name].status == "pass"
          )
          .reduce((a, e) => a && e, true)
        ...
        if (toClose) {
          ...
          if(rule.status === 'pass' && rule.rough && __hasGas(rule.hash)) {
