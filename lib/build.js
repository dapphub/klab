...
const bound = {
  ...
  "int128" : name => `#rangeUInt(128, (${ name }))`, //the word, not the interpreted int
  "int256" : name => `#rangeUInt(256, (${ name }))`, //the word, not the interpreted int
...
const make_abi_dsl = ({type, name}, i) =>
      "#" + type +
      (type == ("int256" || "int128") ? "(#signed(ABI_" + name + "))" : "(ABI_" + name + ")")
  ...
  const getTerm = ({
    ...
    const buildAccount = varname => {
        let storage_pass = Object.keys(act.storage[varname])
            .reduce((a, key) => `addNew(${a}, ${(/^(\d|\#)/.test(key) ? "" : "#" + varname.replace('ACCT_ID',act.subject) + ".") + clean(key)}, (${clean(act.storage[varname][key][0])} => ${clean(act.storage[varname][key][1])}))`, `${varname}_STORE`)
        let storage_fail = Object.keys(act.storage[varname])
            .reduce((a, key) => `addNew(${a}, ${(/^(\d|\#)/.test(key) ? "" : "#" + varname.replace('ACCT_ID',act.subject) + ".") + clean(key)}, ${clean(act.storage[varname][key][0])})`, `${varname}_STORE`)
  ...
  const act_if = Object.keys(act.types)
    ...
    .concat(flatten(Object.keys(act.storage)
      .map(varname => Object.keys(act.storage[varname])
           .map(key => `notBool #inKeys(${varname}_STORE, ${(/^(\d|\#)/.test(key) ? "" : "#" + varname.replace('ACCT_ID',act.subject) + ".") + clean(key)})`)
      )
    ))
    .concat(act.if || [])
