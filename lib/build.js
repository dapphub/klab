  ...
  let storages_blocks = Object
    ...
    .map(subjectName => {
      ...
      let storage = Object
        .keys(act.storage[subjectName])
        .map(key => {
          console.log(subjectName, key, act.creates && act.creates[subjectName]);
...
const buildAct = config => ({act, oog, pass, name, hasGas, gas, lemma}) => {
  ...
  const buildAccount = varname => {
    ...
    Object.keys(act.storage[varname]).forEach(key => {origStorage_vals.push(remember_junk(clean(act.storage[varname][key][0])) + "_PRIME")})
    const origStorage_str = (key) => {
      let lhs = `${storagekeys(realname(varname), key)} <- (${remember_junk(clean(act.storage[varname][key][0]))}_PRIME)`
    ...
    const balance = act.balance
      ...
      || isCreate && "0 => 0"
      || varname + "_balance";
    const code = (isCreate && ".WordStack => " || "") + alias + "_bin_runtime";
    const nonce = (isCreate)
      && "0 => 1"
      || alias + "_nonce => _"
  ...
  const act_if = Object.keys(act.types)
    ...
    .concat(origStorage_vals.map(v => `#rangeUInt(256, ${v})`))
