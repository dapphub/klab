    ...
    Object.keys(act_proofs)
      .forEach(name => {
        ...
        if (toClose) {
          ...
          const module = kjson.renderModule(_rules, gas_module.length > 0, rule.name) // TODO maybe skip this
          const hash   = config.get_proof_hash(module);
          rule.module  = kjson.renderModule(_rules, gas_module.length > 0, hash);
          rule.hash    = hash;
