  ...
  const getTerm = ({
    ...
    const buildAccount = varname => {
        let storage_pass = Object.keys(act.storage[varname])
            .reduce((a, key) => `store(${a}, ${(/^(\d|\#)/.test(key) ? "" : "#" + act.subject + ".") + clean(key)}, (${clean(act.storage[varname][key][0])} => ${clean(act.storage[varname][key][1])}))`, "_")
        let storage_fail = Object.keys(act.storage[varname])
            .reduce((a, key) => `store(${a}, ${clean(key)}, ${clean(act.storage[varname][key][0])})`, "_")
