...
const newCleanup = (act, text) => {
  ...
  const header = `<div class="codeheader">
      ${h(['a', `class="anchor" href="#${id}"`])('anchor')}
...
const __getGas = name => JSON.parse(read(path.join(KLAB_OUT, "gas", name + ".kast")));
...
const getTerm = ({
  ...
  "ethereum.evm.callState.programBytes" : `#parseByteStack("0x${bin_runtime}")`,
  "ethereum.evm.callState.program"      : `#asMapOpCodes(#dasmOpCodes(#parseByteStack("0x${bin_runtime}"), PETERSBURG))`,
...
const buildAct = config => ({act, oog, pass, name, hasGas, gas, gas_raw, splitNumber}) => {
  ...
  var junk = 0;
  const remember_junk = vname => vname == "_" && ("Junk_" + (junk++)) || vname
  ...
  const spec = kjson.renderRule({
    ...
    term: getTerm({
      ...
      bin_runtime: config.contracts[act.subject].bin_runtime,
      oog,
    ...
    Object.keys(act_proofs)
      .forEach(name => {
        ...
        if (toClose) {
          ...
          const module = kjson.renderModule(_rules, false, rule.name) // TODO maybe skip this
          const hash   = config.get_proof_hash(module);
          const gas_names = (rule.file_suffix && [hash] || [])
          rule.module  = kjson.renderModule(_rules, gas_names.length > 0, hash);
          rule.hash    = hash;
