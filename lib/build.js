  ...
  const getTerm = ({
    ...
    const buildAccount = varname => {
        let storage_pass = Object.keys(act.storage[varname])
            .reduce((a, key) => `addNew(${a}, ${(/^(\d|\#)/.test(key) ? "" : "#" + act.subject + ".") + clean(key)}, (${clean(act.storage[varname][key][0])} => ${clean(act.storage[varname][key][1])}))`, `${varname}_STORE`)
        let storage_fail = Object.keys(act.storage[varname])
            .reduce((a, key) => `addNew(${a}, ${clean(key)}, ${clean(act.storage[varname][key][0])})`, `${varname}_STORE`)
