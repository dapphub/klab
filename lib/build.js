...
const buildAct = config => ({act, oog, pass, name}) => {
  ...
  const mu = ListOfList => ListOfList.reduce((a, list) => a.concat(list))
  ...
  const act_if = Object.keys(act.types)
    ...
    .concat(mu(Object.keys(act.storage)
      .map(varname => storagekeys(varname)
         .map(key => `notBool #inKeys(${varname}_STORE, ${key})`)
      )
    ))
    .concat(act.if || [])
