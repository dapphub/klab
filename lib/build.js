...
const proofCollection = config => act_collection => act_collection
  ...
  .map(act => {
    ...
    if (config.splitActs.indexOf(__a2n(act)) > -1) {
      failActs = act.iff.map((_, i) =>
                             [{
                               ...
                               splitNumber: i,
                               gas: act.gas || 3000000,
                               oog: !(config.noOOGActs.indexOf(__a2n(act)) > -1),
                               name: __a2n(act) + "_fail" + ((config.noOOGActs.indexOf(__a2n(act)) > -1) ? '_rough' : '') + i
    ...
    return [{
      ...
      gas: act.gas || 3000000,
      oog: !(config.noOOGActs.indexOf(__a2n(act)) > -1),
      name: __a2n(act) + "_fail" + ((config.noOOGActs.indexOf(__a2n(act)) > -1) ? '_rough' : '')
    }] : []).concat(failActs);
   })
  .reduce((a, cs) => a.concat(cs), [])
      ...
      .forEach(name => {
        ...
        if (toClose) {
          ...
          if(rule.status === 'accept' && rule.rough && __hasGas(rule.hash)) {
            ...
            if(config.noOOGActs.indexOf(name.replace('pass_rough','')) > -1) {
