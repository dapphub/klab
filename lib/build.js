  ...
  const getTerm = ({
    ...
    const buildAccount = varname => {
        ...
        Object.keys(act.storage[varname])
            .forEach(key => {
                prestore = `store(${prestore}, ${clean(key)}, ${clean(act.storage[varname][key][0])})`
            })
        let poststore = "S"
        Object.keys(act.storage[varname])
            .forEach(key => {
                poststore = `store(${poststore}, ${clean(key)}, ${clean(act.storage[varname][key][1])})`
            })
