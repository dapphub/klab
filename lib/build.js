...
const buildAct = config => act => {
  ...
  const accounts = Object.keys(act.storage)
    ...
    if (act.internal) {
        if (!config.contracts[act.subject].ast) {
            warn(`No solidity AST given for ` + act.name + `, cannot extract pc values for internal functions.\n Run solc --ast-json and add the output file to config.`)
        }
