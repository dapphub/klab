...
const makePrelude = config => {
  ...
  fs.writeFileSync(path.join(KLAB_OUT, "prelude.smt2"), smt_prelude)
}
...
`
  ...
  fs.writeFileSync(path.join(KLAB_OUT, "rules.k"), rules_k);
}
...
const buildAct = config => act => {
  ...
  const storage_success = Object.keys(act.storage.ACCT_ID)
    .map(key => _i(key) + " |-> (" + act.storage.ACCT_ID[key].map(_i).join(" => ") + ")" )
    .concat(["_:Map"])
    .join("\n  " + " ".repeat(14))
    + "\n"
  ...
  const storage_fail = Object.keys(act.storage.ACCT_ID)
    .map(key => _i(key) + " |-> (" + _i(act.storage.ACCT_ID[key][0]) + (act.storage.ACCT_ID[key].length > 1 ? " => _ " : "") + ")" )
    .concat(["_:Map"])
    .join("\n  " + " ".repeat(14))
        + "\n"
  ...
  const accounts = Object.keys(act.storage)
    ...
    .map(varname => {
      let fstore = Object.keys(act.storage[varname])
        .map(key => _i(key) + " |-> (" + act.storage[varname][key].map(_i).join(" => ") + ")")
        .concat(["_:Map"])
        .join("\n" + " ".repeat(12))
  ...
  const accounts_fail = Object.keys(act.storage)
    ...
    .map(varname => {
      let fstore = Object.keys(act.storage[varname])
        .map(key => _i(key) + " |-> (" + _i(act.storage[varname][key][0]) + (act.storage[varname][key].length > 1 ? " => _ " : "") + ")" )
        .concat(["_:Map"])
        .join("\n"+" ".repeat(12))
