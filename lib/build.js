...
const buildAct = config => ({act, oog, pass, name}) => {
  ...
  const getTerm = ({
    ...
    pc
      ...
      "k": "#execute ~> CONTINUATION => " + ((act.internal && pass) ? "#execute ~> CONTINUATION" : "#halt ~> CONTINUATION"),
  ...
  // GAS
  const gas = "VGas => " + (act.hasGas ? "VGas -Int " + act.gas : "_");
...
//    .filter(varname => varname != act.subject)
  ...
  const wordstack = pass && wordstack_success || wordstack_fail;  
