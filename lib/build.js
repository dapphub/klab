  ...
  const getTerm = ({
    ...
    const buildAccount = varname => {
        ...
        Object.keys(act.storage[varname])
            .forEach(key => {
                storage_pass = `store(${storage_pass}, ${(/^(\d|\#)/.test(key) ? "" : "#" + act.subject + ".") + clean(key)}, (${clean(act.storage[varname][key][0])} => ${clean(act.storage[varname][key][1])}))`
            })
        let storage_fail = "_"
        Object.keys(act.storage[varname])
            .forEach(key => {
                storage_fail = `store(${storage_fail}, ${clean(key)}, ${clean(act.storage[varname][key][0])})`
            })
