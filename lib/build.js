...
const proofCollection = config => act_collection => act_collection
  ...
  .map(act => {
    let failActs = [];
    if (config.splitActs.indexOf(__a2n(act)) > -1) {
      failActs = act.iff.map((_, i) =>
                             [{
                               act: {...act},
                               pass: false,
                               splitNumber: i,
                               gas: act.gas || 3000000,
                               oog: (config.OOGActs.indexOf(__a2n(act)) > -1),
                               name: __a2n(act) + "_fail" + ((config.OOGActs.indexOf(__a2n(act)) == -1) ? '_rough' : '') + i
    ...
    }].concat(("iff" in act && config.splitActs.indexOf(__a2n(act)) == -1) ? [{
      act: {...act},
      pass: false,
      splitNumber: -1,
      gas: act.gas || 3000000,
      oog: (config.OOGActs.indexOf(__a2n(act)) > -1),
      name: __a2n(act) + "_fail" + ((config.OOGActs.indexOf(__a2n(act)) == -1) ? '_rough' : '')
    }] : []).concat(failActs);
   })
  .reduce((a, cs) => a.concat(cs), [])
...
const buildAct = config => ({act, oog, pass, name, hasGas, gas, gas_raw, splitNumber}) => {
  ...
  var pass_junk = 0;
  var fail_junk = 0;
  const remember_pass_junk = vname => {
    var ret = vname;
    if(vname == "_") {
      ret = "Junk_" + pass_junk;
      pass_junk = pass_junk + 1;
    }
    return ret;
  }
  const remember_fail_junk = vname => {
    var ret = vname;
    if(vname == "_") {
      ret = "Junk_" + fail_junk;
      fail_junk = fail_junk + 1;
    }
    return ret;
  }
  const buildAccount = varname => {
    //TODO - do both in one go and unite junk into one counter?
    let storage_pass = Object.keys(act.storage[varname])
      .map(key => `[${storagekeys(realname(varname), key)} <- (${remember_pass_junk(clean(act.storage[varname][key][0]))} => ${clean(act.storage[varname][key][1])})]`)
      .join('        \n');
    let storage_fail = Object.keys(act.storage[varname])
      .map(key => `[${storagekeys(realname(varname), key)} <- (${remember_fail_junk(clean(act.storage[varname][key][0]))} => _)]`)
      .join('        \n');
  ...
  const act_if = Object.keys(act.types)
    ...
    .concat((new Array(pass_junk)).fill(1).map((a, i) => `#rangeUInt(256, Junk_${i})`))
  ...
  const cond_fail = (act_if || [])
    .map(c => "  andBool " + c)
        .join("\n")
        + (act.iff ? ((splitNumber == -1) ?
       "\n  andBool notBool (\n            "
       + act.iff
       .reduceRight(brackedJoin("\n    andBool "), "")
       + "\n  )"
      :
      "\n" + act.iff.slice(0, splitNumber).concat(act.iff.slice(splitNumber+1)).map(c => "  andBool " + c).join('\n')
       +  "\n  andBool notBool (\n            "
       + act.iff[splitNumber]
       + "\n  )"
           ) :  [])
      ...
      .forEach(name => {
        ...
        if (toClose) {
          ...
          if(rule.status === 'accept' && rule.rough && __hasGas(rule.hash)) {
            ...
            if(config.OOGActs.indexOf(name.replace('_pass_rough','')) > -1) {
