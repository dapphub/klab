...
const buildAct = config => ({act, oog, pass, name, hasGas, gas, gas_raw, callDepth = "_"}) => {
  ...
  const getTerm = ({
    ...
    "ethereum.evm.callState.callDepth": "VCallDepth => " + callDepth,
...
const buildActs = (config, act_collection) => {
  ...
  const forkPass = (obj, rule) => {
    let fork_obj = {
      ...
      callDepth: __getBehaviour("callDepth", rule.hash),
