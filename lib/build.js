...
const buildAct = config => ({act, oog, pass, name, hasGas, gas, lemma}) => {
  ...
  const buildAccount = varname => {
   ...
   // This means that we're assuming the LHS of the term has the same value as it did at the beginning of the transaction.
   // This assumption doesn't hold when the storage location has been modified in another call within a transaction.
   // When that happens, claims about gas for this storage location won't be completely accurate.
   // This could be solved by making this value symbolic, which is how it used to be.
    const origStorage_str = Object.keys(act.storage[varname])
      .map(key => `[${storagekeys(realname(varname), key)} <- (${remember_junk(clean(act.storage[varname][key][0]))})]`)
      .join('        \n');
  ...
  // OUTPUT
  const buildReturns = ( rs ) => rs.length > 0 ? rs.map(ret => `#buf(32, ${mapInterface(act.interface, toK(ret))})`).join(' ++ ') : ".WordStack"
  const output = pass
    && !oog
