    ...
    Object.keys(act_proofs)
      .forEach(name => {
        ...
        if (toClose) {
          ...
          const suffix = (rule.file_suffix || "")
            + rule.ctx
              .filter(r => r.file_suffix)
              .map(r => r.file_suffix)
              .join('\n\n')
          const _rules = [rule.spec].concat(rule.ctx.map(r => r.spec  + "\n[trusted]\n"))
          const module = kjson.renderModule(_rules, suffix, rule.name) // TODO maybe skip this
          const hash   = config.get_proof_hash(module);
          rule.module  = kjson.renderModule(_rules, suffix, hash);
          rule.hash    = hash;
