...
const buildAct = config => ({act, oog, pass, name, hasGas, gas, lemma}) => {
  ...
  const buildAccount = varname => {
    ...
    if (!isCreate) {Object.keys(act.storage[varname]).forEach(key => {origStorage_vals.push("PRIME_" + remember_junk(clean(act.storage[varname][key][0])))})}
    const origStorage_str = (key) => {
      let lhs = `${storagekeys(realname(varname), key)} <- (PRIME_${remember_junk(clean(act.storage[varname][key][0]))})`
