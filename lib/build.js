...
const makeInterabiExhaustiveness = (alias, cname, contract) => {
  ...
  const module = kjson.renderModule([spec], false, id)
...
const __getGasRaw = name => JSON.parse(read(path.join(KLAB_OUT, "gas", name + ".raw.kast")));
...
const getTerm = ({
  ...
  "ethereum.network.activeAccounts"     : activeAccounts + " => _",
  "ethereum.network.accounts"           : accounts,
  "ethereum.evm.callState.static"       : isStatic ? '_' : 'false'
})

const buildAct = config => ({act, oog, pass, name, hasGas, gas, gas_raw, splitNumber, lemma}) => {
...
const buildActs = (config, act_proofs) => {
  ...
  const forkPass = (obj, rule) => {
    ...
    const gas_raw = __getGasRaw(rule.hash)
    let fork_obj = {
      ...obj,
      act: rule.act,
      pass: true,
