...
const {format}      = require("../lib/kast.js");
...
const buildAct = config => ({act, oog, pass, name}) => {
  ...
  const getTerm = ({
    ...
    const buildAccount = varname => {
        let storage = `${varname}_STORE_PRE => ${varname}_STORE_POST`;
//        let storage_pass = Object.keys(act.storage[varname])
//            .reduce((a, key) => `store(${a}, ${(/^(\d|\#)/.test(key) ? "" : "#" + act.subject + ".") + clean(key)}, (${clean(act.storage[varname][key][0])} => ${clean(act.storage[varname][key][1])}))`, `${varname}_STORE`)
//        let storage_fail = Object.keys(act.storage[varname])
//            .reduce((a, key) => `store(${a}, ${clean(key)}, ${clean(act.storage[varname][key][0])})`, `${varname}_STORE`)
//        let storage = (pass && !oog) ? storage_pass : (storage_fail + '=> _')
  ...
  const storagekeys = varname => Object.keys(act.storage[varname]).map(key => (/^(\d|\#)/.test(key) ? "" : "#" + act.subject + ".") + clean(key))
  ...
  // GAS
  const gas = "VGas => " + ((act.hasGas && !oog) ? format(act.gas_raw, true) : "_");
  let gasCond;
  if (pass) {
  ...
  const act_if = Object.keys(act.types)
    ...
    .concat(flatten(Object.keys(act.storage)
                    .map(varname => flatten(Object.keys(act.storage[varname])
                                            .map(key => 
                                                 [`select(${varname}_STORE_PRE, ${clean(key)}) ==K ${clean(act.storage[varname][key][0])}`, `${varname}_OTHER =/=K ${clean(key)}`])
                                           )
                        )))
    .concat(act.if || [])
  ...
  const cond = pass
    ...
    act.ensures =  (pass || !oog) ? (flatten(Object.keys(act.storage)
                          .map(varname => flatten(Object.keys(act.storage[varname])
                                                .map(key => [`select(${varname}_STORE_PRE, ${varname}_OTHER) ==K select(${varname}_STORE_POST, ${varname}_OTHER)`, `select(${varname}_STORE_POST, ${clean(key)}) ==K ${clean(act.storage[varname][key][1])}`])))
                                            )) : []
        .concat(
            (!pass || oog) ? ("FAILURE =/=K EVMC_SUCCESS") : []
        )
        .concat(act.ensures || [])
  // PC
