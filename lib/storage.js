          ...
          subjectName => {
              let alphaKeys = Object.keys(act.storage[subjectName]).map(
                  key => {
                      ...
                      let args = getStorageDef(key_).getArgs(key_);
              ...
              let collisions = {};
              indices.forEach(key => {
                  collisions[key] = alphaKeys.filter(e => e[0] == key)
              })
              Object.keys(collisions).forEach(key => {
              if (collisions[key].length > 1) {
                  warn(`Warning! Potential collision in storage index ${key}. Arguments ${collisions[key].map(s => s[1]).join(' and ')} may be equal.`)
              }
              })
          })
    }
