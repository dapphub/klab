  ...
  Object.keys(json.implementations)
    .forEach(alias => {
      ...
      if (dapp) {
        ...
        bin_runtime = json.implementations[name].bin_runtime ? fs.readFileSync(json.implementations[name].bin_runtime).toString().split('Binary of the runtime part: ').slice(-1)[0].trim() : bin_runtime
      }
