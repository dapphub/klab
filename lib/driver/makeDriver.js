...
const make = (msg, ch) => {
  ...
  // TODO - rewrite this to own format, not spec.ini
  let spec_file = msg.spec + `\n\n[pgm]\ncompiler: "Solidity"\ncode: "0x${msg.contracts[msg.behaviour_of].bin_runtime.trim()}"`
  ...
  //If trusted lemmas are to be added
  if (msg.trust) {
     let gen_rules = spawnSync("python3", [
         path.join(res_path, "gen-rules.py"),
         path.join(res_path, "spec-tmpl.k"),
         path.join(wd_path, `spec.ini`),
     ].concat(tasks));
     if (!(gen_rules.stderr.toString() == "")) {
         console.log(gen_rules.stderr.toString());
         ch({
             type: "error",
             data: gen_rules.stderr.toString(),
             peer: msg.peer
         });
     }
     let k_rule_buffer = gen_rules.stdout;
     //Make all rules trusted:
     let trustspec = msg.trustSpec.replace("attribute:","attribute: [trusted]")
         + `\n\n[pgm]\ncompiler: "Solidity"\ncode: "0x${msg.trustBin.trim()}"`
     let trust_tasks = Object.keys(ini.parse(trustspec));
     fs.writeFileSync(path.join(wd_path, `trust_spec.ini`), trustspec);
     const gen_trust_rules = spawnSync("python3", [
         path.join(res_path, "gen-rules.py"),
         path.join(res_path, "spec-tmpl.k"),
         path.join(wd_path, `trust_spec.ini`),
     ].concat(tasks));
     if (!(gen_trust_rules.stderr.toString() == "")) {
         console.log(gen_trust_rules.stderr.toString());
         ch({
             type: "error",
             data: gen_trust_rules.stderr.toString(),
             peer: msg.peer
         });
     }
     k_rule_buffer += gen_trust_rules.stdout;
     fs.writeFileSync(path.join(wd_path, `rules`), k_rule_buffer);
     const gen_module = spawnSync("python3", [
         path.join(res_path, "gen-module.py"),
         path.join(res_path, "module-tmpl.k"),
         `proof`,
         path.join(wd_path, "rules")
     ]);
     if (!(gen_module.stderr.toString() == "")) {
         console.log(gen_module.stderr.toString());
         ch({
             type: "error",
             data: gen_module.stderr.toString(),
             peer: msg.peer
         });
     }
     k_spec_file_buffer = gen_module.stdout;
  }
  else {
     gen_spec = spawnSync("python3", [
     ...
     if (!(gen_spec.stderr.toString() == "")) {
         console.log(gen_spec.stderr.toString());
         ch({
             type: "error",
             data: gen_spec.stderr.toString(),
             peer: msg.peer
         });
     }
     k_spec_file_buffer = gen_spec.stdout;
  }
