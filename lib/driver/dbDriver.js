...
const getBlob = (proofid, nodeid) => {
  const output_path = fs.readFileSync(path.join(KLAB_WD_PATH, proofid, "OUTPUT_PATH"))
    .toString()
    .trim()
  return JSON.parse(fs.readFileSync(path.join(output_path, `nodes/${nodeid}.json`)).toString());
}
...
const getrule = (msg, ch) => {
  const output_path = fs.readFileSync(path.join(KLAB_WD_PATH, proofid, "OUTPUT_PATH"))
    .toString()
    .trim()
  let blob_path = path.join(output_path, `/nodes/${msg.rule}.json`)
...
const subscribe = (msg, ch) => {
  const output_path = fs.readFileSync(path.join(KLAB_WD_PATH, msg.proofid, "OUTPUT_PATH"))
    .toString()
    .trim()
  const tstream = ts.createReadStream(path.join(output_path, "debugg.log"));
