...
module.exports = function (state) {
  ...
  const createPrefix = isCreate ? "bin_" : "bin_runtime_";

  // console.log(bin_runtime);
//  console.log(state.config.implementations)
//  console.log(Object.keys(state.config.v2n))
  let contract_o = state.config.implementations[call_id];
  ...
  // STORAGE
//  let storage_str = kast.format(storage_object);
//  let storage_str = JSON.stringify(storage_object, false, 2)

  // PC + OPs
  let inst = contract[createPrefix + "pc_to_inst_map"][pc];
  let from = Math.max(0, inst - 5);
  let to   = Math.min(contract[createPrefix + 'instructions'].length, from + 10);
  let pc_op_str = contract[createPrefix + "instructions"]
    .slice(from, to)
    .map((s, i) => (from + i === inst ? "> " : "  ") + r(hex(contract[createPrefix + "inst_to_pc"][from + i])) + "  " + r(hex(from + i)) + "  " + (s.length > 20 ? s.slice(0, 20) + "..." : s))
