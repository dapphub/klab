...
module.exports = ({CLI, onion, Remote, Settings}) => {
  ...
  .map(msg => {
    return (prev) => {
      ...
      if(msg.type === "error") updated.error = prev.error + "\n" + msg.data
      if(msg.type === "rstep") {
        let step = msg.data[0];
        let from = msg.data[1];
        let to = msg.data[2];
      ...
      if(msg.type === "node") {
        ...
        let constraint = msg.data.node.constraint;
      ...
      if(msg.type === "circcdata") {
        ...
        let rhs = circc.rhs.replace("\\n", "\n").split("\n").map(s => s.trim()).join("\n");
        updated.circc[id] = _.uniq([formatConstraint(circc.rhs)].concat(updated.circc[id]));
      }
