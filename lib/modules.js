...
const view = {
  ...
  gas : {
    view: state => {
      let behavior = genBehaviour(state);
      let finished = JSON.stringify(state.finished, false, 2)
      ...
      const clean = str => str.replace(/VGas\ \-Int\ /g, "")
        // TODO - clean `VGas -Int ` to get exact numbers
        // k.children.reduce((_else, ck) => {
        //   const scond = ck.deltaCRaw.trim()
        //   const sthen = getGasExpr(ck);
        //   const selse = _else == "" ? "" : `#else (${_else}) `;
        //   return `(#if (${scond}) #then (${sthen}) ${selse}#fi)`;
        // }, "")

      let gas = clean(getGasExpr(behavior));
      fs.writeFileSync(`out/gas_${state.config.name}.log`, gas);
      // return JSON.stringify(behavior, false, 2);
      return gas;
    }
  },
  sourcemap: {
