...
const view = {
  ...
  wasm: {
    view: (state) => {
      ...
      let nod = state.nodes[id];
      let k = kast.format(kast.get(nod, "k"));
//      console.log(k)
      let locals = kast.format(kast.get(nod, "curFrame.locals"));
      let valstack = kast.format(kast.get(nod, "valstack"));
      return clc.bold("K:\n") + k.toString() + '\n'
        + clc.bold("Locals:\n") + locals + '\n'
        + clc.bold("Valstack:\n") + valstack.split(': ').join('\n').replace('(','').replace(')','').replace('.ValStack','') + '\n'
    },
  },
  // TODO - evm specific
  evm: {
