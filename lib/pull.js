...
const pull = ({K, onion}) => {
  ...
  const toConstraintRequest$ = onion.state$
    ...
    .map(state => state.config.proofid + " " + state.path[state.path.length - 1].step.to.split("_")[0])
    .compose(dropRepeats())
  ...
  const toTermRequest$ = onion.state$
    ...
    .map(state => state.config.proofid + " " + state.path[state.path.length - 1].step.to.split("_")[1])
    .compose(dropRepeats())
    .map(id => ({type:"getnode", data: id}))
  ...
  const circcRequest$ = K
    ...
    .compose(sampleCombine(onion.state$))
    .map(([msg, state]) => ({type:"getcircc", data: state.config.proofid + " " + msg.data[0]}))
  ...
  const z3feedbackRequest$ = K
    ...
    .compose(sampleCombine(onion.state$))
    .map(([msg, state]) => ({type:"getz3feedback", data: state.config.proofid + " " + msg.data[0] + " " + msg.data[1]}))
