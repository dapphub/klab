...
module.exports = ({ CLI, onion }) => {
  const cmd_mode_enter$ = CLI
    ...
    .filter(([key, state]) => !state.cmd_mode && key.sequence == ":")
    .map(([key, _]) => key)
    .map(key => state => {
  ...
  const cmd_mode_write$ = CLI
    .compose(sampleCombine(onion.state$))
    .filter(([key, state]) => !!state.cmd_mode && /^[a-zA-Z0-9\-\_\.\ ]$/.test(key.sequence) && key.sequence != "\r")
  ...
  const cmd_exec$ = CLI
    .compose(sampleCombine(onion.state$))
    .filter(([key, state]) => state.cmd_mode && key.sequence == "\r");
