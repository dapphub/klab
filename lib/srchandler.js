...
// EXPORT
const getCodeStringFromPc = (config, pc) => {
  let hide_color = 244;
  let show_color = 255;
  let pc_to_inst_map = get_pc_to_inst_map(config.bin_runtime);
  // console.log("pc", pc, "inst", pc_to_inst_map[pc], "srcmap", srcmapArr[pc_to_inst_map[pc]]);
  var cpos = srcmapArr(config.sourcemap)[pc_to_inst_map[pc]];
  ...
  var str = (hide(config.source.slice(0, cpos.s)) + show(config.source.slice(cpos.s, cpos.s + cpos.l)) + hide(config.source.slice(cpos.s + cpos.l)));
