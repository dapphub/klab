...
const MIN_PUSH_OPCODE = 96
const MAX_PUSH_OPCODE = 127
const isPushOpcode  = opcode => opcode >= MIN_PUSH_OPCODE && opcode <= MAX_PUSH_OPCODE
const getPushLength = opcode => opcode - MIN_PUSH_OPCODE + 1

// Instructions
const get_pc_to_inst_map = bin_string => {
  const opcodes = []
  for(let i=0; i<bin_string.length; i+=2) {
    opcodes.push(parseInt(bin_string.slice(i, i+2), 16));
  }
...
let getNodeSrcAst = (ast, name, argTypes) => {
  return ast.map(node => {
    if (node.attributes && node.attributes.name && node.attributes.name == name) {
      ...
      if (deepEqual(fArgTypes,argTypes))
      {
        return node.src;
      }
    }
    if (node.children) {
      return getNodeSrcAst(node.children, name, argTypes).join('');
    }
  })
}
...
let functionNameToPcRange = (sig, srcmapArr, ast, bin_runtime, inst_to_pc) => {
  ...
  let srcFromAst = getNodeSrcAst([ast], name, argTypes)[0];
...
let unrollBranches = (bin, start_pc, pushed_pc, depth, max_depth) => {
  ...
  if (isPushOpcode(next_op_num)) {
    let n = getPushLength(next_op_num)
