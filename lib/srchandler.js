...
let functionNameToPcRange = (name, srcmapArr, ast, bin_runtime) => {
    ...
    let end_index = Number(start_index) + Number(j_index);
    const inst_to_pc = get_pc_to_inst_map(bin_runtime).instructions.map((inst, i) => get_pc_to_inst_map(bin_runtime).pc_to_inst_map.indexOf(i));
