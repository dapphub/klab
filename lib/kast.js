...
const format = (o, isRaw = false, mixFix = false) => {
  if(o && o.node == KVARIABLE) {
    o = (/^(.*)_\d+\:\w+$/).test(o.originalName)
      ? (/^(.*)_\d+\:\w+$/).exec(o.originalName)[1]
      : o.originalName
      ;
  } else if(o && o.node == KTOKEN) {
    const pt = {
    ...
    let tokenMap = {
      ...
      ".IntList": "",
      "i32_WASM-DATA": "",
      "(local.tee_)_WASM": "local.tee ",
      "(local.get_)_WASM": "local.get ",
      "(local.set_)_WASM": "local.set ",
      "init_locals__WASM": "init_locals ",
      "init_local___WASM": "init_local",
      "#init_locals___WASM": "#init_locals ",
      "[_]_WASM-DATA": "[] ",
      "add_WASM": "add ",
      "div_u_WASM": "div_u ",
    ...
    let trifix = {
      "label_{_}__WASM": ["label"," { "," } "],
      "(if_(then_)(else_))_WASM": ["if "," then "," else "]
    }
    let weirdFix = {
      "_.____WASM": " . "
    }
    let fixif = {
      "(if__else_end)_WASM": ["if "," else "," end "]
    }
    if((!isRaw) && o.label in infix) {
      o = "(" + childs.join(infix[o.label]) + ")"
    }
    else if((!isRaw) && o.label in trifix) {
      o = trifix[o.label][0] + childs[0] + trifix[o.label][1] + childs[1] + trifix[o.label][2] + childs[2];
    } else if((!isRaw) && o.label in weirdFix) {
      o = childs[0] + weirdFix[o.label] + childs[1] + " " + childs[2] + " " + childs[3];
    } else if((!isRaw) && o.label in fixif) {
      o = fixif[o.label][0] + childs[0] + " " + childs[1] + fixif[o.label][1] + childs[2] + fixif[o.label][2];
