...
const antimatch = (pattern, term, path = []) => {
  if (pattern.node == KVARIABLE) {
    ...
    // TODO - introduce a type arg which states if arg is missing or rejected
    ...
    const matching = matching_keys
      .map(([key, pattern_]) => antimatch(pattern_, rhs.find(el => el[0] == key)[1], path.concat( key.replace(/\n\s*/g, "") )))
